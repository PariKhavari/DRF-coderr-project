...
class OfferWriteSerializer(serializers.ModelSerializer):
    ...
    def validate_details(self, value: list) -> list:
        ...
        if self.instance is None:
            if len(value) != 3:
                raise serializers.ValidationError("An offer must contain exactly three details.")
        ...
        if len(value) == 0:
            raise serializers.ValidationError("At least one detail must be provided for update.")
        for item in value:
            if not item.get("offer_type"):
                raise serializers.ValidationError("Each detail must include offer_type for update.")
        return value
    ...
    def _update_details(self, offer: Offer, details_data: list) -> None:
        ...
        for detail_data in details_data:
            ...
            try:
                ...
                raise serializers.ValidationError({"detail": f"No detail with offer_type '{offer_type}' exists."})
...
#     def validate_details(self, value: list) -> list:
#         """[DE] Prüft, dass genau drei Detail-Varianten existieren. [EN] Ensures exactly three detail variants exist."""
#         if len(value) != 3:
#             raise serializers.ValidationError("An offer must contain exactly three details.")
#         types = {item.get("offer_type") for item in value}
#         required = {OfferDetail.TYPE_BASIC, OfferDetail.TYPE_STANDARD, OfferDetail.TYPE_PREMIUM}
...
#     def update(self, instance: Offer, validated_data: dict) -> Offer:
#         """[DE] Aktualisiert Offer und ggf. Details. [EN] Updates offer and optionally its details."""
#         details_data = validated_data.pop("details", None)
#         for attr, value in validated_data.items():
#             setattr(instance, attr, value)
#         instance.save()
...
#     def _update_details(self, offer: Offer, details_data: list) -> None:
#         """[DE] Aktualisiert bestehende OfferDetails anhand ihres Typs. [EN] Updates existing offer details by type."""
#         for detail_data in details_data:
#             offer_type = detail_data.get("offer_type")
#             if offer_type is None:
#                 raise serializers.ValidationError({"offer_type": "offer_type is required."})
...
#     def to_representation(self, instance: Offer) -> dict:
#         """[DE] Gibt Offer mit vollständigen Details zurück. [EN] Returns offer with full details."""
#         data = super().to_representation(instance)
#         data["details"] = OfferDetailSerializer(instance.details.all(), many=True).data
#         return data
